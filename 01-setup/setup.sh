...
echo "🔁 Assigning 'Key Vault Secrets Officer' role to external user on Key Vault..."
...
echo "🔁 Assigning 'Contributor' role to Service Principal on Resource Group..."
az role assignment create \
  --assignee "$sp_id" \
  --role "Contributor" \
  --scope "/subscriptions/$subscription_id/resourceGroups/$resource_group" || echo "⚠️ Role may already exist."

echo "🔁 Assigning 'Contributor' role to Service Principal on Subscription (optional - wide access)..."
az role assignment create \
  --assignee "$sp_id" \
  --role "Contributor" \
  --scope "/subscriptions/$subscription_id" || echo "⚠️ Role assignment for subscription may already exist."
